# @yourname/lite-carousel

**Ultra lightweight React carousel/slider**  
فقط **~2.4 kB** (gzipped) · Zero dependencies · SSR safe · TypeScript · Tailwind ready

پشتیبانی کامل از:
- Slide & Fade انیمیشن
- Autoplay + pause on hover
- چند اسلاید همزمان (slidesToShow)
- Responsive breakpoints
- Infinite loop
- Touch / Mouse drag
- Keyboard navigation
- Next.js, Vite, CRA — همه جا کار می‌کنه

[Live Demo →](https://stackblitz.com/edit/lite-carousel-demo)  
[npm →](https://www.npmjs.com/package/@yourname/lite-carousel)  
[GitHub →](https://github.com/yourname/lite-carousel)

## نصب

```bash
npm install @yourname/lite-carousel
# یا
yarn add @yourname/lite-carousel
# یا
pnpm add @yourname/lite-carousel